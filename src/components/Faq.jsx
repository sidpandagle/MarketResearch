                ...
                <div className="max-w-3xl mx-auto mt-8 space-y-4 md:mt-8">
                    {
                        faqs.map((faq, key) => {
                            return (
                                <div key={key} onClick={() => setSelectedFaqId(faq.id)} className={`transition-all duration-200 bg-white border border-gray-200 ${faq.id === selectedFaqId && 'shadow-md'} cursor-pointer hover:bg-gray-50`}>
                                    <button type="button" className="flex items-center justify-between w-full px-4 py-5 sm:p-6">
                                        <span className="flex text-lg font-semibold text-black mr-4"> {faq.question} </span>
