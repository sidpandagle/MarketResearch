...
export default class PressRelease extends Component {
  render() {
    const heading = 'Press Release';
    const scrollToTop = () => {
      window.scroll(0, 0)
    }
    return (
      <div>
        <div className="mb-6 md:text-3xl text-lg h-[200px] md:h-[300px] font-extrabold flex items-center justify-center bg-gradient  text-white">{heading.toUpperCase()}</div>
                  ...
                  <div className='border rounded-md p-4 sticky top-[20px]'>
                    ...
                    <div className='flex flex-col gap-2'>
                      {categories.map((res, key) => {
                        return (
                          // <Link key={key} to={`/heading/${res}`} onClick={scrollToTop}>
                          // </Link>
